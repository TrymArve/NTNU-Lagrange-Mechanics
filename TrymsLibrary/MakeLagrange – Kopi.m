...
if ~isstruct(sys)
    ...
    elseif string(sys) == "config"
        ...
        config.compareMCG = "The extracted model will be compared to the EL-equations and the result is printed as a message. (to verify the validity of the matrices) (if they do not match, then the system could contain  higher exponents than 1 and 2 for the ddq and dq)";
    ...
    elseif string(sys) == 'fields'
        ...
        fields.mass = "Assign your own names to the masses, indices correspond to the positions";
...
% for j = 1:N
% for i = 1:n
%     P{j} = subs(P{j}, diff(qt{i}, t), dq{i});
%     K{j} = subs(K{j}, diff(qt{i}, t), dq{i});
%     P{j} = subs(P{j}, qt{i}, q{i});
%     K{j} = subs(K{j}, qt{i}, q{i});
% end
% Ptot = simplify( Ptot + P{j} );
% Ktot = simplify( Ktot + K{j} );
% end
...
Lagrange.help.P   = "The total kinectic energy of the system.";
Lagrange.help.K   = "The total potential energy of the system.";
if isfield(sys, 'subDiffs') && sys.subDiffs == 1
...
% COMPARE SYSTEM MODEL TO EULER LAGRANGE MODEL
% (Make sure no errors have occured)
% % % if isfield(sys, 'compareMCG') && sys.compareMCG == 1
% % %     SM = simplify( EL_M*[ddq{:}].' + EL_C*[dq{:}].' + EL_G );
% % %     Equal = simplify( SM - EL )
% % %     if isempty(find(Equal ~= 0, 1)) == 1
% % %         disp('Euler Lagrange equations ARE equal to system model!')
% % %     else
% % %         disp('WARINING: Euler Lagrange equations are NOT equal to system model')
% % %     end
% % % end
